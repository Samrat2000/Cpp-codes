...
class Matrix
{
	...
	public :
		...
		inline void add(Matrix &m1,Matrix &m2);
		inline void sub(Matrix &m1,Matrix &m2);
		void mult(Matrix &m1,Matrix &m2);
};

inline void Matrix::add(Matrix &m1,Matrix &m2)
{
	for(int i=0;i<r;i++)
	{
		for(int j=0;j<c;j++)
		{
			cout << setw(4) << m1.a[i][j] + m2.a[i][j] << "  ";
		}
		cout << endl;
	}
}

inline void Matrix::sub(Matrix &m1,Matrix &m2)
{
	for(int i=0;i<r;i++)
	{
		for(int j=0;j<c;j++)
...
int main()
{
	...
	m2.display();
	//Matrix sum(r1,c1) , subtract(r1,c1) 
	Matrix prod(r1,c2);
	cout << "1.Addition \n2.Subtraction \n3.Multiplication \nEnter your choice : ";
	cin >> c;
